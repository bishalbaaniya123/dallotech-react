...
class MaterialDropZone extends React.Component {
  ...
  onDrop(filesVal) {
    ...
    if(this.state.files.length <= 1){
      if(this.props.uploadType === 'KYC'){
        const data = new FormData();
        data.append('kyc', filesVal[0]);
        api.uploadKyc(data)
          .then(res => {
            this.props.getFileName(res.file_name)
          })
      }
    }
  ...
  render() {
    ...
    const previews = filesArray => filesArray.map((file, index) => {
      ...
      if (isImage(file)) {
        return (
          <Tooltip title={file.name} key={index.toString()}>
            <div>
    ...
    return (
      <div>
        ...
        <div className="row preview" style={this.props.previewStyle}>
...
MaterialDropZone.propTypes = {
  ...
  uploadType: PropTypes.string.optional,
  getFileName: PropTypes.object.optional,
};
